...
if(isset($_POST['submit'])){
    ...
    if ( $leases->getPropertytype()=="") {
        ...
        $leasesDBController = new leasesDBController();
        $leasesDBController->db = $db;	
		$miscRentData = $leasesDBController->getRentsRecord($leases->getId());
		
		$_GET['id'] = $leases->getId();
	}
}
...
$propertyData = $dataSourceController->getPropertyTypeDataSource(3,1,array(20));
...
$proppics = array();
if($leases->getID()!=""){
    $sql = "SELECT CONCAT(FK_ReportID,'/', photopath) as image, caption FROM propphotos
...
require("templates/templateRent.php");
