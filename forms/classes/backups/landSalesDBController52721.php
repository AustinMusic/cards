...
//
    ...
    public function getRecord($id){
        ...
        $sql_fields[] = SALES_TRANSACTIONS_TABLE.".confirm_by";
    ...
    public function landsaleOperation($landsales,$user_id){
        ...
        try{
            ...
            if($actionMode=="INSERT"){
                $stmt = $this->db->prepare("INSERT ".PROPERTY_TABLE." (FK_ReportID, property_name, address, streetnumber, streetname, city, county, state, zip_code, lat, lng, propertytype,
                    propertysubtype, msa, genmarket, submarkid, legal_desc, photo1, thumbnail, gla_inputtype, gla_inputsize, gross_land_acre, gross_land_sf, unusable_sf, unusable_acre,
                    unusable_desc, net_usable_sf, net_usable_acre, net_usable_desc, shape, topography, frontage, depth, utilities, flood_plain, orientation, site_access, exposure, easement,
                    easement_desc, other_land_comments, change_zoning, zoning_juris, zoning_code, zoning_desc, inc_far, fut_zoning_code, fut_zoning_juris, fut_zoning_desc, rezone_time,
                    max_building_ht, floor_area_ratio, max_floor_area, traffic_count, traffic_date, side_street, inter_street, distance_direction, rural_electricity, well_septic, municiple_sewer,
                    municiple_water, natural_gas, telephone, cable_tv, proposed_building, ground_lease)
                    VALUES (:FK_ReportID,:property_name,:address,:streetnumber,:streetname,:city,:county,:state,:zip_code,:lat,:lng,:propertytype,:propertysubtype,:msa,:genmarket,:submarkid,:legal_desc,
                    :photo1,:thumbnail,:gla_inputtype,:gla_inputsize,:gross_land_acre,:gross_land_sf,:unusable_sf,:unusable_acre,:unusable_desc,:net_usable_sf,:net_usable_acre,:net_usable_desc,
                    :shape,:topography,:frontage,:depth,:utilities,:flood_plain,:orientation,:site_access,:exposure,:easement,:easement_desc,:other_land_comments,:change_zoning,:zoning_juris,
                    :zoning_code,:zoning_desc,:inc_far,:fut_zoning_code,:fut_zoning_juris,:fut_zoning_desc,:rezone_time,:max_building_ht,:floor_area_ratio,:max_floor_area,:traffic_count,
                    :traffic_date,:side_street,:inter_street,:distance_direction,:rural_electricity,:well_septic,:municiple_sewer,:municiple_water,:natural_gas,:telephone,:cable_tv,:proposed_building,
                    :ground_lease) ");
            }else{
                $stmt = $this->db->prepare("UPDATE ".PROPERTY_TABLE." SET property_name=:property_name, address=:address, streetnumber=:streetnumber, streetname=:streetname,
                    city=:city, county=:county, state=:state, zip_code=:zip_code, lat=:lat, lng=:lng, propertytype=:propertytype, propertysubtype=:propertysubtype,
                    msa=:msa, genmarket=:genmarket,submarkid=:submarkid,  legal_desc=:legal_desc,photo1=:photo1, thumbnail=:thumbnail, gla_inputtype=:gla_inputtype, gla_inputsize=:gla_inputsize,
                    gross_land_acre=:gross_land_acre,gross_land_sf=:gross_land_sf, unusable_sf=:unusable_sf, unusable_acre=:unusable_acre, unusable_desc=:unusable_desc, net_usable_sf=:net_usable_sf,
                    net_usable_acre=:net_usable_acre,net_usable_desc=:net_usable_desc, shape=:shape, topography=:topography, frontage=:frontage, depth=:depth, utilities=:utilities,
                    flood_plain=:flood_plain, orientation=:orientation,site_access=:site_access, exposure=:exposure, easement=:easement, easement_desc=:easement_desc,
                    other_land_comments=:other_land_comments, change_zoning=:change_zoning,zoning_juris=:zoning_juris, zoning_code=:zoning_code, zoning_desc=:zoning_desc, inc_far=:inc_far,
                    fut_zoning_code=:fut_zoning_code, fut_zoning_juris=:fut_zoning_juris,fut_zoning_desc=:fut_zoning_desc, rezone_time=:rezone_time, max_building_ht=:max_building_ht,
                    floor_area_ratio=:floor_area_ratio, max_floor_area=:max_floor_area,traffic_count=:traffic_count, traffic_date=:traffic_date, side_street=:side_street, inter_street=:inter_street,
                    distance_direction=:distance_direction,rural_electricity=:rural_electricity, well_septic=:well_septic, municiple_sewer=:municiple_sewer, municiple_water=:municiple_water,
                    natural_gas=:natural_gas, telephone=:telephone,cable_tv=:cable_tv, proposed_building=:proposed_building, ground_lease=:ground_lease
            ...
            $stmt->bindValue(':frontage',$landsales->getFrontage(),PDO::PARAM_STR);
            $stmt->bindValue(':depth',$landsales->getDepth(),PDO::PARAM_STR);
            $stmt->bindValue(':utilities',$landsales->getUtilities(),PDO::PARAM_INT);
            ...
            $stmt->bindValue(':side_street',$landsales->getSide_street(),PDO::PARAM_STR);
            $stmt->bindValue(':inter_street',$landsales->getInter_street(),PDO::PARAM_STR);
            $stmt->bindValue(':distance_direction',$landsales->getDistance_direction(),PDO::PARAM_STR);
            ...
            if($actionMode=="INSERT"){
                $stmt = $this->db->prepare("INSERT INTO ".GROUND_LEASE_TABLE." (FK_ReportID, gl_development_name, gl_year_built, gl_total_project, gl_leased_land_sf, gl_building_footprint, gl_anchor_tenants,
                    gl_major_tenants, gl_status, gl_start_date, gl_end_date, gl_options_period, gl_escalations, gl_length, gl_rent_begin, gl_rent_per_sf_land, gl_rent_per_sf_footprint, gl_fee_simple_equiv,
                    gl_fee_equiv_per_sf, gl_rate_return, adj_price_sf_pad)
                    VALUES  (:FK_ReportID, :gl_development_name, :gl_year_built, :gl_total_project, :gl_leased_land_sf, :gl_building_footprint, :gl_anchor_tenants, :gl_major_tenants, :gl_status,
                    :gl_start_date, :gl_end_date, :gl_options_period, :gl_escalations, :gl_length, :gl_rent_begin, :gl_rent_per_sf_land, :gl_rent_per_sf_footprint, :gl_fee_simple_equiv, :gl_fee_equiv_per_sf,
                    :gl_rate_return, :adj_price_sf_pad)");
            }else{
                $stmt = $this->db->prepare("UPDATE ".GROUND_LEASE_TABLE." SET gl_development_name=:gl_development_name, gl_year_built=:gl_year_built, gl_total_project=:gl_total_project,
                    gl_leased_land_sf=:gl_leased_land_sf, gl_building_footprint=:gl_building_footprint, gl_anchor_tenants=:gl_anchor_tenants, gl_major_tenants=:gl_major_tenants,
                    gl_status=:gl_status,gl_start_date=:gl_start_date,gl_end_date=:gl_end_date,gl_options_period=:gl_options_period,gl_escalations=:gl_escalations,gl_length=:gl_length,gl_rent_begin=:gl_rent_begin,
                    gl_rent_per_sf_land=:gl_rent_per_sf_land,gl_rent_per_sf_footprint=:gl_rent_per_sf_footprint,gl_fee_simple_equiv=:gl_fee_simple_equiv,gl_fee_equiv_per_sf=:gl_fee_equiv_per_sf,
                    gl_rate_return=:gl_rate_return,adj_price_sf_pad=:adj_price_sf_pad
                    WHERE ".GROUND_LEASE_TABLE.".FK_ReportID=:FK_ReportID");
            }
            ...
            if($actionMode=="INSERT"){
                $stmt = $this->db->prepare("INSERT INTO ".LAND_TABLE." (FK_ReportID, type_land_radio, type_residential_land, unit_lot_type, no_lots, unadj_sale_price, lot_complete_date, fut_avg_home_price,
                    fut_avg_home_price2, lot_home_price_ratio, unit_density_net_acre, hard_cost_lot, off_site_imp, fut_finish_size_range, fut_finish_size_avg, value_entitle, inc_entitlement,
                    value_entitlement_lot, site_amenities, proj_fut_avg_finish_price, adj_price_finish_with, sale_price_lot_with, sale_price_net_acre_with, price_net_sf_with, adj_price_finished_wo,
                    sale_price_lot_wo, sale_price_net_acre_wo, price_net_sf_wo, ind_lot_nos, finish_lot_size_range, finish_lot_size_sfba, finish_lot_size_sf, lot_topography, project_amenities,
                    original_price, other_lot_comment, bulk_price_lot, bulk_price_sf_avg, adj_bulk_sale_price, ind_sale_pct_discount, report_price_lot)
                    VALUES  (:FK_ReportID, :type_land_radio, :type_residential_land, :unit_lot_type, :no_lots, :unadj_sale_price, :lot_complete_date, :fut_avg_home_price, :fut_avg_home_price2,
                    :lot_home_price_ratio, :unit_density_net_acre, :hard_cost_lot, :off_site_imp, :fut_finish_size_range, :fut_finish_size_avg, :value_entitle, :inc_entitlement, :value_entitlement_lot,
                    :site_amenities, :proj_fut_avg_finish_price, :adj_price_finish_with, :sale_price_lot_with, :sale_price_net_acre_with, :price_net_sf_with, :adj_price_finished_wo, :sale_price_lot_wo,
                    :sale_price_net_acre_wo, :price_net_sf_wo, :ind_lot_nos, :finish_lot_size_range, :finish_lot_size_sfba, :finish_lot_size_sf, :lot_topography, :project_amenities, :original_price,
                    :other_lot_comment, :bulk_price_lot, :bulk_price_sf_avg, :adj_bulk_sale_price, :ind_sale_pct_discount, :report_price_lot)");
            }else{
                $stmt = $this->db->prepare("UPDATE ".LAND_TABLE." SET type_land_radio=:type_land_radio, type_residential_land=:type_residential_land, unit_lot_type=:unit_lot_type,
                    no_lots=:no_lots, unadj_sale_price=:unadj_sale_price, lot_complete_date=:lot_complete_date, fut_avg_home_price=:fut_avg_home_price, fut_avg_home_price2=:fut_avg_home_price2,
                    lot_home_price_ratio=:lot_home_price_ratio, unit_density_net_acre=:unit_density_net_acre, hard_cost_lot=:hard_cost_lot, off_site_imp=:off_site_imp, fut_finish_size_range=:fut_finish_size_range,
                    fut_finish_size_avg=:fut_finish_size_avg, value_entitle=:value_entitle, inc_entitlement=:inc_entitlement, value_entitlement_lot=:value_entitlement_lot, site_amenities=:site_amenities,
                    proj_fut_avg_finish_price=:proj_fut_avg_finish_price, adj_price_finish_with=:adj_price_finish_with, sale_price_lot_with=:sale_price_lot_with, sale_price_net_acre_with=:sale_price_net_acre_with,
                    price_net_sf_with=:price_net_sf_with, adj_price_finished_wo=:adj_price_finished_wo, sale_price_lot_wo=:sale_price_lot_wo, sale_price_net_acre_wo=:sale_price_net_acre_wo,
                    price_net_sf_wo=:price_net_sf_wo, ind_lot_nos=:ind_lot_nos, finish_lot_size_range=:finish_lot_size_range, finish_lot_size_sfba=:finish_lot_size_sfba, finish_lot_size_sf=:finish_lot_size_sf,
                    lot_topography=:lot_topography, project_amenities=:project_amenities, original_price=:original_price, other_lot_comment=:other_lot_comment, bulk_price_lot=:bulk_price_lot,
                    bulk_price_sf_avg=:bulk_price_sf_avg, adj_bulk_sale_price=:adj_bulk_sale_price, ind_sale_pct_discount=:ind_sale_pct_discount, report_price_lot=:report_price_lot
                    WHERE ".LAND_TABLE.".FK_ReportID=:FK_ReportID");
            }
            ...
            if($actionMode=="INSERT"){
                $stmt = $this->db->prepare("INSERT INTO ".SALES_TRANSACTIONS_TABLE." (FK_ReportID, property_appraised, mc_job, appraiser_name, appraiser_name_two, grantor, grantee, land_alloc_sale, sale_price, less_alloc_imp_price, improve_value_source, alloc_land_value, prop_rights_conv, interest_conv, conv_doc_type, conv_doc_rec_no, record_date, record_date_two, sale_status, market_time, current_use, proposed_use_change, proposed_use_desc, fee_equiv_price, fee_simple_equiv_price, cash_equiv_price, type_finance, list_price_avail, list_price, list_price_change, confirm_by, inspect_date, confirm_date, inspect_type, mc_file_no, confirm_1_source, relationship_1, confirm_2_souce, relationship_2, market_flyer, fin_term_adj, fin_term_adj_desc, cond_sale_adj, cond_sale_desc, demo_cost, demo_cost_desc, onsite_extra_cost, onsite_extra_cost_desc, offsite_develop, offsite_develop_desc, broker_cost, broker_cost_desc, eff_sale_price_stab, adj_price_comment, inc_ffe, value_ffe, desc_ffe, adj_price_acre_gross, adj_price_acre_net, adj_price_sf_gross, adj_price_sf_net, adj_price_sf_far, sale_comments, conf_comments)
            ...
            $stmt->bindValue(':confirm_by',$landsales->getConfirm_by(),PDO::PARAM_STR);
