var _aa_map_tpl_generalMarket = function(options){
	var aa_map_tpl_generalMarket;
	var rbe_engine = new _rbe_engine();
	var aa_engine = new _aa_engine();
	
	aa_map_tpl_generalMarket = {
		options:{
			controller: "",
			mapObject: false,
			...
			template: "<div class='filters form-group' style='margin-bottom:4px;'>"+
							...
							"<input type='checkbox' id='_typeValue' data-id='_typeid' data-filter='gmid' filter-type='group' class='generalMarket _mapFilter'/>_typeValue"+
						"</div>",
			order: 1,
			dataSource: "",
			layout:{
		...
		render: function(){
			...
			aa_mapRenderer.renderCheckBoxGroup({value:"gmid",title:"genmarket"});
			this.assignEvents();
		},
		assignEvents: function(){
			...
			if(rbe_engine.checkElement(_elements)){
				for(var i=0;i<_elements.length;i++){
					_elements[i].setAttribute("onClick",this.options.controller+".setSubMarket(this);");
				}
			}
		},
		setSubMarket: function(_element){
			...
			var marketID = _element.getAttribute("data-id");
			...
			if(this.options.connectedTemplate!==false){
				this.options.connectedTemplate.setSubMarket(_element,hasChecked,marketID);
			}
	...
	aa_map_tpl_generalMarket.setOptions(options);
	
	return aa_map_tpl_generalMarket;
};