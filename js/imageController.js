				...
				$("#dialogueDeleteImage").dialog({
					...
					buttons: [{
							...
							click: function () {
								...
								$.ajax({
									url: "../forms/ajax_deleteImage.php",
									type: "post",
									dataType: "json",
									data: {
									...
									success: function (retJson) {
										if (retJson[0].code == "001" || retJson[0].code == "002") {
											if (rbe_engine.hasClass(_melement.parentNode.parentNode.parentNode,"remove")) {
												...
												var __element = _melement.parentNode.parentNode.querySelector("#preview-sp1");
												if (rbe_engine.checkElement(__element)) {
													__element.parentNode.parentNode.innerHTML = '<img id="preview-sp1" src="../comp_images/no-photo.jpg" width="100%" style="cursor:pointer;"/>';
												}
												...
												if (rbe_engine.checkElement(c_element)) {
													...
													__element = c_element.querySelector("#photo1path");
													if (rbe_engine.checkElement(__element)) {
														__element.value = "";
													}
													__element = c_element.querySelector("#thumbpath");
													if (rbe_engine.checkElement(__element)) {
														__element.value = "";
													}
												}
									...
									error: function (jqXHR, textStatus, errorThrown) {
										alert("An whoopsie just occured.");
									}
								});
							}
						}
