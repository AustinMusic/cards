...
$spreadsheet = \PhpOffice\PhpSpreadsheet\IOFactory::load('retailpadrow.xlsx');
...
if ( $result->num_rows > 0 ) {
	while ( $subprop = $result->fetch_object() ) {
		...
		$spreadsheet->getActiveSheet()->setCellValue('E154', $subprop->submarket);
		...
		$spreadsheet->getActiveSheet()->setCellValue('E147', $subprop->estapp);
	}
}
	...
	if ( $result->num_rows > 0 ) {
		...
		while ( $record = $result->fetch_object() ) {
			$spreadsheet->getActiveSheet()
				...
				->setCellValueByColumnAndRow($column, 177, "'" . $record->gl_length)
...
header('Content-Disposition: attachment;filename="Retail-Pad ROW Land Sales.xlsx"');
