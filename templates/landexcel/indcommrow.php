...
$spreadsheet = \PhpOffice\PhpSpreadsheet\IOFactory::load('indcommrow.xlsx');
...
if ( $result->num_rows > 0 ) {
	while ( $subprop = $result->fetch_object() ) {
		...
		$spreadsheet->getActiveSheet()->setCellValue('E155', $subprop->submarket);
...
WHERE a.id in (".implode(',',(array)$ids).") order by FIELD (a.id,".$ids.")" ) ) {
	...
	if ( $result->num_rows > 0 ) {
		...
		while ( $record = $result->fetch_object() ) {
			...
			->setCellValueByColumnAndRow($column, 215, "'" . $record->mc_file_no)
...
header('Content-Disposition: attachment;filename="Industrial - Commercial ROW Land Sales.xlsx"');
