...
if ( $result = $conn->query( "select a.id, a.reportname, UPPER(b.property_name) as cappropname from report a left join property b on b.FK_ReportID = a.id where a.id = " . $_GET['id'] . "" ) ) {
  ...
  $templateProcessor = new\ PhpOffice\ PhpWord\ TemplateProcessor( 'insvalrepcostnew.docx' );
...
header( "Content-Disposition: attachment; filename=Insurable Replacement Cost New.docx" );
readfile( $temp_file );
unlink( $temp_file );
