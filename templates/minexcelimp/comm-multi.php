...
LEFT OUTER join WFDictionary s on s.id = g.park_quality WHERE a.id in (".implode(',',(array)$ids).") order by FIELD (a.id,".$ids.")" ) ) {
	...
	$spreadsheet->getActiveSheet()->getStyle('W185')
            ->getNumberFormat()
...
header('Content-Disposition: attachment;filename="Commercial-Multi.xlsx"');
