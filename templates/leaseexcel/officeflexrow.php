...
$spreadsheet = \PhpOffice\PhpSpreadsheet\IOFactory::load('officeflexrow.xlsx');
$ids = $_GET['id'];
$job = $_GET['job'];
	...
	if ( $result->num_rows > 0 ) {
		...
		while ( $record = $result->fetch_object() ) {
			$spreadsheet->getActiveSheet()
				...
				->setCellValueByColumnAndRow($column, 167, "'" . $record->mc_file_no)
...
header('Content-Disposition: attachment;filename="Flex Office ROW Leases.xlsx"');
