...
if ( $result = $conn->query( "SELECT @rn:=@rn+1 AS rank, saletype, CONCAT(price,' per Unit', adjusted, saletype2, record_date) as price, sale_comments as salecomments
	FROM (
  	SELECT FORMAT(adj_price_unit,0) as price, date_format(record_date,'%c/%y') as record_date, IF(sale_status = 185,'Listing ','Sale ') as saletype, IF(sale_status = 185,'Asking ',IF(sale_status = 181,'Exprired ',IF(sale_status = 182,'Sale-Fail ',IF(sale_status = 183,'Pending ',IF(sale_status = 186,'Purchase Option ',IF(sale_status = 188,'Terminated ','')))))) as saletype2, sale_comments, IF(fin_term_adj + cond_sale_adj + excess_surplus_value + defer_maint_cost + add_ti_cost + broker_cost + stabil_cost <>0,' - Adjusted; ','; ') as adjusted
  	FROM saletrans
