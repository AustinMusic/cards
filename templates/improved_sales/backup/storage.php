...
require_once '../includes/impsale/storage_mysqli.php';
...
if(!boolval($_GET['showPreview'])){
	...
	try {
		...
		$dir = __DIR__ . '/';
        $rid = $_GET['id'];
        $download->setDocTemplate('../output/impsale/storage_wordTemplate.docx');
        $download->setXsltSource('../output/impsale/storage.xslt');
        $download->setImageSource('../../comp_images/');
        $download->create('Mini Storage Improved Sales.docx');
    } catch (Exception $e) {
        $error = $e->getMessage();
    }
	exit();
