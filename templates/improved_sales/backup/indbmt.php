...
require_once '../includes/impsale/indbsmt_mysqli.php';
...
if(!boolval($_GET['showPreview'])){
	...
	try {
		...
		$dir = __DIR__ . '/';
        $rid = $_GET['id'];
        $download->setDocTemplate('../output/impsale/indbmt_wordTemplate.docx');
        $download->setXsltSource('../output/impsale/indbmt.xslt');
        $download->setImageSource('../../comp_images/');
        $download->create('Industrial with Basement Improved Sales.docx');
    } catch (Exception $e) {
        $error = $e->getMessage();
    }
	exit();
