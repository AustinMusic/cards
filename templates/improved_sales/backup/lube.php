...
require_once '../includes/impsale/lube_mysqli.php';
...
if(!boolval($_GET['showPreview'])){
	...
	try {
		...
		$dir = __DIR__ . '/';
        $rid = $_GET['id'];
        $download->setDocTemplate('../output/impsale/lube_wordTemplate.docx');
        $download->setXsltSource('../output/impsale/lube.xslt');
        $download->setImageSource('../../comp_images/');
        $download->create('Mini Lube Improved Sales.docx');
    } catch (Exception $e) {
        $error = $e->getMessage();
    }
	exit();
...
if (isset($_POST['download'])) {
    try {
        ...
        $rid = $_GET['id'];
        $download->setDocTemplate('../output/impsale/lube_wordTemplate.docx');
        $download->setXsltSource('../output/impsale/minilube.xslt');
        $download->setImageSource('../../comp_images/');
        $download->create('Mini Lube Improved Sales.docx');
    } catch (Exception $e) {
        $error = $e->getMessage();
