...
require_once '../includes/impsale/commshop_mysqli.php';
...
if(!boolval($_GET['showPreview'])){
	...
	try {
		...
		$dir = __DIR__ . '/';
        $rid = $_GET['id'];
        $download->setDocTemplate('../output/impsale/commshop_wordTemplate.docx');
        $download->setXsltSource('../output/impsale/commshop.xslt');
        $download->setImageSource('../../comp_images/');
        $download->create('Shopping Center Improved Sales.docx');
    } catch (Exception $e) {
        $error = $e->getMessage();
    }
	exit();
