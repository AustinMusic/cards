...
require_once '../includes/impsale/deal_mysqli.php';
...
if(!boolval($_GET['showPreview'])){
	...
	try {
		...
		$dir = __DIR__ . '/';
        $rid = $_GET['id'];
        $download->setDocTemplate('../output/impsale/deal_wordTemplate.docx');
        $download->setXsltSource('../output/impsale/deal.xslt');
        $download->setImageSource('../../comp_images/');
        $download->create('Vehicle Dealership Improved Sales.docx');
    } catch (Exception $e) {
        $error = $e->getMessage();
    }
	exit();
...
if (isset($_POST['download'])) {
    try {
        ...
        $rid = $_GET['id'];
        $download->setDocTemplate('../output/impsale/deal_wordTemplate.docx');
        $download->setXsltSource('../output/impsale/dealership.xslt');
        $download->setImageSource('../../comp_images/');
        $download->create('Vehicle Dealership Improved Sales.docx');
    } catch (Exception $e) {
        $error = $e->getMessage();
