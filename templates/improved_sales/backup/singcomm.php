...
require_once '../includes/impsale/singcomm_mysqli.php';
...
if(!boolval($_GET['showPreview'])){
	...
	try {
		...
		$dir = __DIR__ . '/';
        $rid = $_GET['id'];
        $download->setDocTemplate('../output/impsale/singcomm_wordTemplate.docx');
        $download->setXsltSource('../output/impsale/singcomm.xslt');
        $download->setImageSource('../../comp_images/');
        $download->create('Commercial Single Tenant Improved Sales.docx');
    } catch (Exception $e) {
        $error = $e->getMessage();
    }
	exit();
