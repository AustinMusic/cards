...
require_once '../includes/impsale/carwash_mysqli.php';
...
if(!boolval($_GET['showPreview'])){
	...
	try {
		...
		$dir = __DIR__ . '/';
        $rid = $_GET['id'];
        $download->setDocTemplate('../output/impsale/carwash_wordTemplate.docx');
        $download->setXsltSource('../output/impsale/carwash.xslt');
        $download->setImageSource('../../comp_images/');
        $download->create('Car Wash Improved Sales.docx');
    } catch (Exception $e) {
        $error = $e->getMessage();
    }
	exit();
