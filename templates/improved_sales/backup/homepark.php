...
require_once '../includes/impsale/homepark_mysqli.php';
...
if(!boolval($_GET['showPreview'])){
	...
	try {
		...
		$dir = __DIR__ . '/';
        $rid = $_GET['id'];
        $download->setDocTemplate('../output/impsale/home_wordTemplate.docx');
        $download->setXsltSource('../output/impsale/home.xslt');
        $download->setImageSource('../../comp_images/');
        $download->create('Mobile Home Park Improved Sales.docx');
    } catch (Exception $e) {
        $error = $e->getMessage();
    }
	exit();
...
if (isset($_POST['download'])) {
    try {
        ...
        $rid = $_GET['id'];
        $download->setDocTemplate('../output/impsale/home_wordTemplate.docx');
        $download->setXsltSource('../output/impsale/homepark.xslt');
        $download->setImageSource('../../comp_images/');
        $download->create('Mobile Home Park Improved Sales.docx');
    } catch (Exception $e) {
        $error = $e->getMessage();
