...
if ( $result = $conn->query( "SELECT @rn:=@rn+1 AS rank, CONCAT(adj_price_sf_nra,' per SF') as price, record_date as recdate, saletype, saletype2, sale_comments as salecomments, IF(fin_term_adj + cond_sale_adj + excess_surplus_value + defer_maint_cost + add_ti_cost + broker_cost + stabil_cost <>0,' - Adjusted; ','; ') as adjusted
	FROM (
  	SELECT FORMAT(adj_price_sf_nra,2) as adj_price_sf_nra, date_format(record_date,'%c/%y') as record_date, IF(sale_status = 185,'Listing ','Sale ') as saletype, IF(sale_status = 185,'Asking ','') as saletype2, sale_comments, fin_term_adj, cond_sale_adj,excess_surplus_value, defer_maint_cost, add_ti_cost, broker_cost, stabil_cost
  	FROM saletrans
