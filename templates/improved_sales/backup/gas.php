...
require_once '../includes/impsale/gas_mysqli.php';
...
if(!boolval($_GET['showPreview'])){
	...
	try {
		...
		$dir = __DIR__ . '/';
        $rid = $_GET['id'];
        $download->setDocTemplate('../output/impsale/gas_wordTemplate.docx');
        $download->setXsltSource('../output/impsale/gas.xslt');
        $download->setImageSource('../../comp_images/');
        $download->create('C-Store - Gas Station Improved Sales.docx');
    } catch (Exception $e) {
        $error = $e->getMessage();
    }
	exit();
