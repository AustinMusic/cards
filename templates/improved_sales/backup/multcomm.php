...
require_once '../includes/impsale/multcomm_mysqli.php';
...
if(!boolval($_GET['showPreview'])){
	...
	try {
		...
		$dir = __DIR__ . '/';
        $rid = $_GET['id'];
        $download->setDocTemplate('../output/impsale/multcomm_wordTemplate.docx');
        $download->setXsltSource('../output/impsale/multcomm.xslt');
        $download->setImageSource('../../comp_images/');
        $download->create('Commercial Multi Tenant Improved Sales.docx');
    } catch (Exception $e) {
        $error = $e->getMessage();
    }
	exit();
