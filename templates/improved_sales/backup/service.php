...
require_once '../includes/impsale/service_mysqli.php';
...
if(!boolval($_GET['showPreview'])){
	...
	try {
		...
		$dir = __DIR__ . '/';
        $download->setDocTemplate('../output/impsale/service_wordTemplate.docx');
        $download->setXsltSource('../output/impsale/service.xslt');
        $download->setImageSource('../../comp_images/');
        $download->create('Service and Repair Improved Sales.docx');
    } catch (Exception $e) {
        $error = $e->getMessage();
    }
	exit();
