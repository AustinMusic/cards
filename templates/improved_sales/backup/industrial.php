...
require_once '../includes/impsale/ind_mysqli.php';
...
if(!boolval($_GET['showPreview'])){
	...
	try {
		...
		$dir = __DIR__ . '/';
        $rid = $_GET['id'];
        $download->setDocTemplate('../output/impsale/ind_wordTemplate.docx');
        $download->setXsltSource('../output/impsale/ind.xslt');
        $download->setImageSource('../../comp_images/');
        $download->create('Industrial Improved Sales.docx');
    } catch (Exception $e) {
        $error = $e->getMessage();
    }
	exit();
...
if (isset($_POST['download'])) {
    try {
        ...
        $rid = $_GET['id'];
        $download->setDocTemplate('../output/impsale/ind_wordTemplate.docx');
        $download->setXsltSource('../output/impsale/industrial.xslt');
        $download->setImageSource('../../comp_images/');
        $download->create('Industrial Improved Sales.docx');
    } catch (Exception $e) {
        $error = $e->getMessage();
