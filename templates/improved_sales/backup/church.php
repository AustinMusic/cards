...
require_once '../includes/impsale/church_mysqli.php';
...
if(!boolval($_GET['showPreview'])){
	...
	try {
		...
		$dir = __DIR__ . '/';
        $rid = $_GET['id'];
        $download->setDocTemplate('../output/impsale/church_wordTemplate.docx');
        $download->setXsltSource('../output/impsale/church.xslt');
        $download->setImageSource('../../comp_images/');
        $download->create('Church School Improved Sales.docx');
    } catch (Exception $e) {
        $error = $e->getMessage();
    }
	exit();
