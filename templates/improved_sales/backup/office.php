...
require_once '../includes/impsale/office_mysqli.php';
...
if(!boolval($_GET['showPreview'])){
	...
	try {
		...
		$dir = __DIR__ . '/';
        $rid = $_GET['id'];
        $download->setDocTemplate('../output/impsale/off_wordTemplate.docx');
        $download->setXsltSource('../output/impsale/office.xslt');
        $download->setImageSource('../../comp_images/');
        $download->create('Office Improved Sales.docx');
    } catch (Exception $e) {
        $error = $e->getMessage();
    }
	exit();
