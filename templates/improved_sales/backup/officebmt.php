...
require_once '../includes/impsale/officebmt_mysqli.php';
...
if(!boolval($_GET['showPreview'])){
	...
	try {
		...
		$dir = __DIR__ . '/';
        $rid = $_GET['id'];
        $download->setDocTemplate('../output/impsale/officebmt_wordTemplate.docx');
        $download->setXsltSource('../output/impsale/officebmt.xslt');
        $download->setImageSource('../../comp_images/');
        $download->create('Office with Basement Improved Sales.docx');
    } catch (Exception $e) {
        $error = $e->getMessage();
    }
	exit();
...
if (isset($_POST['download'])) {
    try {
        ...
        $rid = $_GET['id'];
        $download->setDocTemplate('../output/impsale/officebmt_wordTemplate.docx');
        $download->setXsltSource('../output/impsale/offbsmt.xslt');
        $download->setImageSource('../../comp_images/');
        $download->create('Office with Basement Improved Sales.docx');
    } catch (Exception $e) {
        $error = $e->getMessage();
