...
if ( $result = $conn->query( "SELECT a.id, b.property_name as propname, CONCAT(b.streetnumber,' ',b.streetname) as address, CONCAT(b.city,', ', b.state) as citystate, b.county, b.legal_desc as legal, sm.submarket, st.subtype, IF(b.photo1 = '','no-photo.jpg',CONCAT(a.id,'/',b.photo1)) as image, vlvl.definition as servlev, d.grantor, d.grantee, FORMAT(d.sale_price,0) as saleprice, FORMAT(d.eff_sale_price_stab,0) as effpricestab, FORMAT(d.eff_sale_price_stab,0) as effpricestab2, IF(d.adj_price_comment = '','',CONCAT(' - ',d.adj_price_comment)) as adjcomment, d.type_finance as fintype, p.definition as proprights, DATE_FORMAT(d.record_date, '%c/%e/%Y') as recdate, IF(d.sale_status = 3,'',CONCAT(' ',ss.definition)) as salestatus, d.conv_doc_rec_no as docno, IF(d.conv_doc_type = 3,'',CONCAT(' ',cdt.definition)) as doctype, d.market_time as markettime, c.year_built as yearbt, CONCAT(' ',c.last_renovation) as reno, FORMAT(c.overall_gba,0) as gba, FORMAT(g.veh_showroom_sf,0) as showroomsf, CONCAT(FORMAT(g.veh_showroom_pct,1),'%') as showroompct, c.no_stories as stories, ot.definition as occupancy, c.const_descr as constdesc, bq.definition as bquality, ec.definition as extc, ic.definition as intc, g.veh_tunnel as tunnels, c.other_const_features as constfeat, IF(d.inc_ffe = 2,'Yes','No') as ffeinc, FORMAT(d.value_ffe,0) as valueffe, b.zoning_code as zcode, b.zoning_desc as zdesc, b.primary_acre as primacre, FORMAT(b.primary_sf,0) as primsf, c.land_build_ratio_primary as ltbprim, sa.definition as access, exp.definition as exposure, ori.definition as orientation, c.oe_income_actual_proforma as proforma, c.oe_income_source as isource, FORMAT(c.oe_pgi,0) as pgi, FORMAT(c.pgi_sf_nra,2) as pgisfnra, CONCAT(FORMAT(c.oe_vacany_pct,1), '%') as vacpct, FORMAT(c.oe_vacancy_credit_loss,0) as vaccredloss, FORMAT(c.vacancy_sf_nra,2) as vacsfnra, FORMAT(c.oe_egi,0) as egi, FORMAT(c.egi_sf_nra,2) as egisfnra, CONCAT(FORMAT(c.oe_expence_ratio,1),'%') as expratio, FORMAT(c.oe_expences,0) as expences, FORMAT(c.expence_sf_nra,2) as expsfnra, FORMAT(c.oe_total_noi,0) as noi, FORMAT(c.oe_noi_sf_nra,2) as noisfnra, FORMAT(d.adj_price_tunnel,0) as adjtunnel, FORMAT(d.underlying_land_value_primary,2) as ulandvale, FORMAT(d.alloc_land_value,0) as allocland, FORMAT(d.alloc_imp_value,0) as allocimp, FORMAT(d.alloc_imp_value_sf_gba,2) as allocimpsfgba, CONCAT(FORMAT(d.cap_rate,2),'%') as caprate, FORMAT(d.adj_price_sf_gba,2) as adjsfgba, CONCAT(FORMAT(d.egim,2),'x') as egim, d.sale_comments as salecomments, d.confirm_1_source as source1, d.confirm_2_souce as source2, CONCAT(ap1.firstname, IF(ap1.midname = '','',CONCAT(' ',ap1.midname,' ')), ap1.lastname, IF(ap1.designation = '','',CONCAT(', ',ap1.designation))) as confirm1, DATE_FORMAT(d.confirm_date, '%c/%e/%Y') as confirmdate FROM report a JOIN property b on b.FK_ReportID = a.id LEFT OUTER JOIN building c on c.FK_ReportID = a.id LEFT OUTER JOIN saletrans d on d.FK_ReportID = a.id LEFT OUTER JOIN proptypedetails g on g.FK_ReportID = a.id LEFT OUTER JOIN subtype st on st.id = b.propertysubtype LEFT OUTER JOIN appraisers ap1 on ap1.id = d.confirm1 LEFT OUTER JOIN WFDictionary vlvl on vlvl.id = g.veh_level LEFT OUTER JOIN submarket sm on sm.id = b.submarkid LEFT OUTER JOIN WFDictionary p on p.id = d.prop_rights_conv LEFT OUTER JOIN WFDictionary ss on ss.id = d.sale_status LEFT OUTER JOIN WFDictionary cdt on cdt.id = d.conv_doc_type LEFT OUTER JOIN WFDictionary ot on ot.id = c.occupancy_type LEFT OUTER JOIN WFDictionary bq on bq.id = c.building_quality LEFT OUTER JOIN WFDictionary ec on ec.id = c.ext_condition LEFT OUTER JOIN WFDictionary ic on ic.id = c.int_condition LEFT OUTER JOIN WFDictionary sa on sa.id = b.site_access LEFT OUTER JOIN WFDictionary exp on exp.id = b.exposure LEFT OUTER JOIN WFDictionary ori on ori.id = b.orientation WHERE a.id in (" . $_GET[ 'id' ] . ") group by FIELD(a.id," . $_GET[ 'id' ] . ") order by FIELD(a.id," . $_GET[ 'id' ] . ")" ) ) {
 
  $phpWord = new\ PhpOffice\ PhpWord\ PhpWord();
  $templateProcessor = new\ PhpOffice\ PhpWord\ TemplateProcessor( 'deal.docx' );
  ...
  while ( $record = $result->fetch_object() ) {    
    ...
    $templateProcessor->setValue( 'allocimpsfgba', $record->allocimpsfgba,1);
    ...
    $templateProcessor->setValue( 'allocimp', $record->allocimp,1);
    $templateProcessor->setValue( 'allocland', $record->allocland,1);
    $templateProcessor->setValue( 'ulandvale', $record->ulandvale,1);
    ...
    $templateProcessor->setValue( 'adjtunnel', $record->adjtunnel,1);
...
header( "Content-Disposition: attachment; filename=Vehicle Dealership Improved Sales.docx" );
