...
if ( $result = $conn->query( "SELECT @rn:=@rn+1 AS rank, CONCAT(eff_rent_sf_mo_shell,' per SF - Shell') as price, record_date as startdate, lease_comment as leasecomments, IF(init_annual_rent <> eff_annual_rent, ' - Adjusted; ','; ') as adjusted
	FROM (
  	SELECT FORMAT(eff_rent_sf_mo_shell,3) as eff_rent_sf_mo_shell, date_format(lease_start_date,'%c/%y') as record_date, lease_comment, init_annual_rent, eff_annual_rent
  	FROM leasetrans  
