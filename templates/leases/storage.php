...
$templatePath = dirname(__FILE__).'/storage.docx';
$outputFilename = 'Self Storage Leases.docx';
...
$sql ="select a.id, if(b.photo1 = '','no-photo.jpg',CONCAT(a.id,'/',b.photo1)) AS image, b.property_name as propname, concat(b.streetnumber,' ',b.streetname) as address, b.city, b.state, b.county, o.subtype, p.genmarket, q.submarket, concat(c.year_built,IF(c.last_renovation = '','',c.last_renovation)) as yearbuilt, c.no_building as nobldgs, c.no_stories as stories, c.const_descr as constr, r.definition as quality, s.definition as intc, t.definition as extc, m.ms_total_units as nounits, m.ms_no_vacant_unit as vacant, CONCAT(FORMAT(m.ms_pct_vacant_unit,1),'%') as pctvacancy, m.ms_vacancy_comment as vaccomment, osm.definition as onsitemgr, m.ms_manager_res as mgrres, CONCAT(if(m.ms_coded_access = 2,'Coded Access, ',''), if(m.ms_alarm = 2,'Alarmed Units, ',''), if(m.ms_heated_unit = 2,'Heated Units','')) as siteamenities, m.ms_access_hours as hours, m.ms_concessions as concessions, REPLACE(e.lease_comment,'$','dollarsign') as leasecomments, e.confirm_1_source as source1, e.confirm_2_souce as source2, CONCAT(ap1.firstname, IF(ap1.midname = '','',CONCAT(' ',ap1.midname,' ')), ap1.lastname, IF(ap1.designation = '','',CONCAT(', ',ap1.designation))) as confirm, DATE_FORMAT(e.confirm_date,'%c/%e/%Y') as confdate FROM report a JOIN property b on b.FK_ReportID = a.id LEFT OUTER JOIN building c on c.FK_ReportID = a.id LEFT OUTER JOIN selfstorage m on m.FK_ReportID = a.id LEFT OUTER JOIN leasetrans e on e.FK_ReportID = a.id LEFT OUTER JOIN appraisers ap1 on ap1.id = e.confirm1 join subtype o on o.id = b.propertysubtype join genmarket p on p.id = b.genmarket join submarket q on q.id = b.submarkid join WFDictionary r on r.id = c.building_quality join WFDictionary s on s.id = c.int_condition join WFDictionary t on t.id = c.ext_condition LEFT OUTER JOIN WFDictionary osm on osm.id = m.ms_onsite_mgr where a.id in (" . $_GET[ 'id' ] . ") order by FIELD(a.id," . $_GET[ 'id' ] . ")";
...
if ($result = $conn->query($sql)) {
	...
	for ($m=1; $m <= $result->num_rows; $m++) {
		...
		if($record != NULL) {	
			...
			$templateProcessor->setValue('confirm#'. $m, $record->confirm, 1);
