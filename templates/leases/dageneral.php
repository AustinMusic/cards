...
if ( $result = $conn->query( "SELECT @rn:=@rn+1 AS rank, CONCAT(eff_rent_sf_yr,' per SF') as price, record_date as startdate, lease_comment as leasecomments, IF(init_annual_rent <> eff_annual_rent, ' - Adjusted; ','; ') as adjusted
	FROM (
  	SELECT FORMAT(eff_rent_sf_yr,2) as eff_rent_sf_yr, date_format(lease_start_date,'%c/%y') as record_date, lease_comment, init_annual_rent, eff_annual_rent
  	FROM leasetrans 
