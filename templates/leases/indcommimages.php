...
require_once '../includes/photoSummary_mysqli.php';
...
if(!boolval($_GET['showPreview'])){
	...
	try {
        $options['stripNsplit'] = 'lease_comment';
        $xml = new Xml($result, null, $options);
        $download = new MsWord($xml);
		$dir = __DIR__ . '/';
        $rid = $_GET['id'];
        $download->setDocTemplate('../output/lease/indcomm_wordTemplate.docx');
        $download->setXsltSource('../output/lease/indcommimages.xslt');
...
if (isset($_POST['download'])) {
    try {
		...
		$dir = __DIR__ . '/';
        $rid = $_GET['id'];
        $download->setDocTemplate('../output/leases/indcomm_wordTemplate.docx');
        $download->setXsltSource('../output/leases/indcommimages.xslt');
        $download->setImageSource('../../comp_images/');
