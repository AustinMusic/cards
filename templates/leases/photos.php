...
$outputFilename = 'Lease Comp Photos.docx';
...
for ($r = 0; $r <= $result->num_rows; $r= $r+2) {
    ...
    if (isset($values[$r])) {
        ...
        \PhpOffice\PhpWord\Shared\Html::addHtml($cell2, '<p style="text-align:center; font-family: Calibri;"><b>Rent Comp ' . $values[$r]['rank'] . '</b><br/>'.$values[$r]['address'] . ', '  . $values[$r]['city'].'</p>');
    }
    $table->addCell(300)->addText('');
    if (isset($values[$r+1])) {
        $cell2 = $table->addCell(3600, array('align' => 'center'));
        \PhpOffice\PhpWord\Shared\Html::addHtml($cell2, '<p style="text-align:center; font-family: Calibri;"><b>Rent Comp ' . $values[$r + 1]['rank'] . '</b><br/>'.$values[$r+1]['address'] . ', '  . $values[$r+1]['city'].'</p>');
    }
}
