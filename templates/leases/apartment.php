...
$templatePath = dirname(__FILE__).'/apartment.docx';
$outputFilename = 'Multi-family Apartment Leases.docx';
...
if ($result = $conn->query($sql)) {
	...
	for ($m=1; $m <= $result->num_rows; $m++) {
		...
		if($record != NULL) {	
			...
			/*$templateProcessor->setValue('appfee#'. $m, $record->appfee);
			$templateProcessor->setValue('movefee#'. $m, $record->movefee);
			$templateProcessor->setValue('refund#'. $m, $record->refund);
			$templateProcessor->setValue('concession#'. $m, $record->concession); */
