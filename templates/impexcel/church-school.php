...
$spreadsheet = \PhpOffice\PhpSpreadsheet\IOFactory::load('church-school.xlsx');
...
LEFT OUTER join WFDictionary s on s.id = g.park_quality WHERE a.id in (".implode(',',(array)$ids).") order by FIELD (a.id,".$ids.")" ) ) {
	...
	while ( $record = $result->fetch_object() ) {
		$spreadsheet->getActiveSheet()
			...
			->setCellValueByColumnAndRow($column, 236, "'" . $record->mc_file_no)
...
header('Content-Disposition: attachment;filename="Church - School Improved Sales.xlsx"');
