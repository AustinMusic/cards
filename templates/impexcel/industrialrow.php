...
$spreadsheet = \PhpOffice\PhpSpreadsheet\IOFactory::load('industrialrow.xlsx');
...
if ( $result->num_rows > 0 ) {
	while ( $subprop = $result->fetch_object() ) {
	  ...
	  $spreadsheet->getActiveSheet()->setCellValue( 'E205', $subprop->submarket );
	  ...
	  $spreadsheet->getActiveSheet()->setCellValue( 'E169', $subprop->footprint );
	  $spreadsheet->getActiveSheet()->setCellValue( 'W186', $subprop->effdov );
...
if ( $result->num_rows > 0 ) {
	...
	while ( $record = $result->fetch_object() ) {
		...
		->setCellValueByColumnAndRow($column, 252, "'" . $record->mc_file_no)
...
$spreadsheet->getActiveSheet()->getRowDimension('214')->setRowHeight(-1);

$spreadsheet->getActiveSheet()->getStyle('W186')->getNumberFormat()->setFormatCode(\PhpOffice\PhpSpreadsheet\Style\NumberFormat::FORMAT_DATE_XLSX14);
...
header('Content-Disposition: attachment;filename="Industrial ROW Improved Sales.xlsx"');
