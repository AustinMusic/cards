...
if ( $result->num_rows > 0 ) {
	while ( $subprop = $result->fetch_object() ) {
	  ...
	  $spreadsheet->getActiveSheet()->setCellValue( 'E208', $subprop->submarket );
...
if ( $result->num_rows > 0 ) {
	...
	while ( $record = $result->fetch_object() ) {
		...
		$spreadsheet->getActiveSheet()
			...
			->setCellValueByColumnAndRow($column, 152, $record->sale_comments)//str_replace("’","'",$record->sale_comments)
...
header('Content-Disposition: attachment;filename="Industrial Improved Sales.xlsx"');
