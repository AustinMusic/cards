...
$spreadsheet = \PhpOffice\ PhpSpreadsheet\ IOFactory::load( 'comm-single.xlsx' );
...
if ( $result->num_rows > 0 ) {
	while ( $subprop = $result->fetch_object() ) {
	  ...
	  $spreadsheet->getActiveSheet()->setCellValue( 'E207', $subprop->submarket );
...
if ( $result = $conn->query( "SELECT a.id, b.property_name, CONCAT(b.streetnumber,' ',b.streetname) as address, b.city, b.state, c.record_date, if(n.definition = '---',' ', n.definition) as sale_status, c.sale_price, c.eff_sale_price_stab, c.alloc_land_value, c.adj_price_comment, c.type_finance, o.definition as prop_rights_conv, concat(' ',c.market_time) as market_time, d.year_built, d.last_renovation, d.no_units, g.total_space, g.other_building_desc, g.no_single_wide, g.no_double_wide, g.no_triple_wide, g.no_rv_space, d.overall_gba, d.overall_nra, f.shop_inline_sf, f.ind_total_office, f.veh_showroom_sf, d.basement_type, d.total_basement_gba, d.total_basement_nra, d.storage_basement_sf, f.veh_tunnel, d.parking_ratio, d.floor_1_gba, d.no_building, no_stories, d.const_descr, p.definition as building_quality, q.definition as building_class, s.definition as park_quality, r.definition as park_condition, t.definition as ext_condition, u.definition as elevator_service, f.ind_clear_height, f.ind_truck_grade, f.ind_truck_dock, v.definition as ind_fire, w.definition as rail_served, f.store_canopy_desc, f.store_no_fuel, d.other_const_features, b.primary_sf, g.bedroom_ratio, g.parking_ratio_unit, b.zoning_code, x.subtype, y.definition as occupancy_type, f.shop_anchor_tenant, f.shop_other_tenant, b.traffic_count, f.store_avg_month_gallon, f.store_month_store_sales, d.oe_pgi, d.oe_vacancy_credit_loss, d.oe_other_income, d.oe_expences, d.oe_total_noi, (d.oe_vacany_pct/100) as oe_vacany_pct, c.store_sales_mult, c.underlying_land_value_primary, c.confirm_1_source, c.relationship_1, CONCAT(ai.firstname,IF(ai.midname = '',' ',CONCAT(' ',ai.midname,' ')),ai.lastname, IF(ai.designation = '','',CONCAT(', ',ai.designation))) as confirm_by, c.confirm_date, c.mc_file_no, c.sale_comments FROM report a JOIN property b on b.FK_ReportID = a.id LEFT OUTER JOIN saletrans c on c.FK_ReportID = a.id LEFT OUTER JOIN building d on d.FK_ReportID = a.id LEFT OUTER join proptypedetails f on f.FK_ReportID = a.id LEFT OUTER join impunit g on g.FK_ReportID = a.id LEFT OUTER JOIN appraisers ai on ai.id = c.confirm1 LEFT OUTER join WFDictionary y on y.id = d.occupancy_type LEFT OUTER join WFDictionary n on n.id = c.sale_status LEFT OUTER join WFDictionary o on o.id = c.prop_rights_conv LEFT OUTER join WFDictionary p on p.id = d.building_quality LEFT OUTER join WFDictionary q on q.id = d.building_class LEFT OUTER join WFDictionary t on t.id = d.ext_condition LEFT OUTER join subtype x on x.id = b.propertysubtype LEFT OUTER join WFDictionary u on u.id = f.off_elevator_service LEFT OUTER join WFDictionary v on v.id = f.ind_fire LEFT OUTER join WFDictionary w on w.id = f.ind_rail LEFT OUTER join WFDictionary r on r.id = g.park_cond LEFT OUTER join WFDictionary s on s.id = g.park_quality WHERE a.id in (" . implode( ',', ( array )$ids ) . ") order by FIELD (a.id,".$ids.")" ) ) {
    ...
    if ( $result->num_rows > 0 ) {
        ...
        while ( $record = $result->fetch_object() ) {
            $spreadsheet->getActiveSheet()
				...
				->setCellValueByColumnAndRow( $column, 32, $record->no_single_wide )
				->setCellValueByColumnAndRow( $column, 33, $record->no_double_wide )
				->setCellValueByColumnAndRow( $column, 34, $record->no_triple_wide )
				->setCellValueByColumnAndRow( $column, 35, $record->no_rv_space )
				...
				->setCellValueByColumnAndRow( $column, 224, "'" . $record->mc_file_no )
...
$spreadsheet->getActiveSheet()->getRowDimension('212')->setRowHeight(-1);
...
header( 'Content-Disposition: attachment;filename="Commercial Single Tenant Improved Sales.xlsx"' );
