<?php
if ($_GET['apptwo'] !=''){
    $templatePath = dirname(__FILE__).'/sumrentstudyindusttwo.docx';
    } else {
    $templatePath = dirname(__FILE__).'/sumrentstudyindustone.docx';
}
$outputFilename = 'Rent Study - Industrial.docx';
...
FROM report a LEFT JOIN property b on b.FK_ReportID = a.id JOIN appraisalinfo c on c.FK_ReportID = a.id JOIN building d on d.FK_ReportID = a.id JOIN proptypedetails e on e.FK_ReportID = a.id JOIN WFDictionary bq on bq.id = d.building_quality JOIN WFDictionary bc on bc.id = d.int_condition join propertytype o on o.id = b.propertytype join subtype p on p.id = b.propertysubtype join WFDictionary q on q.id = b.site_access join WFDictionary r on r.id = b.orientation join WFDictionary s on s.id = b.exposure WHERE a.id = " . $_GET['id'] . "")) {
    ...
    if($record != NULL) {
        ...
        $templateProcessor->setValue( 'offbo', $record->offbo );
        ...
        $templateProcessor->setValue( 'zcode', $record->zcode );
        $templateProcessor->setValue( 'zdesc', $record->zdesc );
        $templateProcessor->setValue( 'zjuris', $record->zjuris );
