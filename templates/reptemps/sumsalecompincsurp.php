<?php
if ($_GET['apptwo'] !=''){
    $templatePath = dirname(__FILE__).'/sumsalecompincsurptwo.docx';
    } else {
    $templatePath = dirname(__FILE__).'/sumsalecompincsurpone.docx';
}
$outputFilename = 'Summary Form - Sales Comp and Income with Surplus Land.docx';
...
if ($result = $conn->query("SELECT a.id, UPPER(b.property_name) as cappropname, b.property_name as propname, b.owner_name as owner, a.reportname, CONCAT(b.streetnumber,' ',b.streetname) as address, CONCAT(b.city,', ',b.state,'  ',b.zip_code) as citystatezip, b.county, IF(b.photo1 = '','../../comp_images/no-photo.jpg',CONCAT('../../comp_images/', a.id,'/',b.photo1)) as image, DATE_FORMAT(a.DueDate,'%M %e, %Y') as DueDate, DATE_FORMAT(c.inspect_date,'%M %e, %Y') as insDate, c.client_reference_no as clientref, c.borrower, DATE_FORMAT(c.eff_date_value,'%M %e, %Y') as effdov, format(d.overall_gba,0) as gba, format(b.net_usable_sf,0) as netsf, format(b.net_usable_acre,2) as netacre, b.zoning_code, b.zoning_desc, concat(d.year_built,' ',d.last_renovation) as yearbt, d.const_descr, d.land_build_ratio_primary as ltbrp, CONCAT(d.site_cover_primary,'%') as scrprim, o.propertytype, p.subtype, IF(d.building_quality = 3,'',bq.definition) as quality, IF(d.int_condition = 3,'',bc.definition) as bcond, FORMAT(d.parking_stalls,0) as pspaces, d.parking_ratio_nra as pratio
FROM report a 
...
JOIN WFDictionary bq on bq.id = d.building_quality 
JOIN WFDictionary bc on bc.id = d.int_condition 
join propertytype o on o.id = b.propertytype
join subtype p on p.id = b.propertysubtype WHERE a.id = " . $_GET['id'] . "")) {
    ...
    if($record != NULL) {
        ...
        $templateProcessor->setValue( 'propertytype', $record->propertytype );
