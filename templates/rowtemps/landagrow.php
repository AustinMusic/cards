...
LEFT OUTER JOIN WFDictionary me on me.id = b.rural_electricity 
LEFT OUTER JOIN WFDictionary mw on mw.id = b.municiple_water 
LEFT OUTER JOIN WFDictionary ms on ms.id = b.municiple_sewer
LEFT OUTER JOIN WFDictionary mws on mws.id = b.well_septic
LEFT OUTER JOIN WFDictionary wr on wr.id = b.waterrights
LEFT OUTER JOIN WFDictionary dr on dr.id = b.dwellrights
WHERE a.id in (" . $_GET[ 'id' ] . ") group by FIELD(a.id," . $_GET[ 'id' ] . ") order by FIELD(a.id," . $_GET[ 'id' ] . ")" ) ) {
 
  $phpWord = new\ PhpOffice\ PhpWord\ PhpWord();
  $templateProcessor = new\ PhpOffice\ PhpWord\ TemplateProcessor( 'landagrow.docx' );
  ...
  while ( $record = $result->fetch_object() ) {    
    ...
    $templateProcessor->setValue( 'adjsp', $record->adjsp,1);
    ...
    $templateProcessor->setValue( 'waterrights', $record->waterrights,1);
    $templateProcessor->setValue( 'irigwell', $record->irigwell,1);
    $templateProcessor->setValue( 'dwellrights', $record->dwellrights,1);
    $templateProcessor->setValue( 'homesite', $record->homesite,1);      
...
header( "Content-Disposition: attachment; filename=Eminent Domain - Agriculture - Land Comps.docx" );
