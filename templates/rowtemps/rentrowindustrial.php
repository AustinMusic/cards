...
if ( $result = $conn->query( "SELECT a.id, b.property_name as propname, CONCAT(b.streetnumber,' ',b.streetname) as address, CONCAT(b.city,', ', b.state) as citystate, b.county, IF(b.photo1 = '','no-photo.jpg',CONCAT(a.id,'/',b.photo1)) as image, gm.genmarket, sm.submarket, pt.propertytype as proptype, st.subtype, c.year_built as yearbt, CONCAT(' ',c.last_renovation) as reno, FORMAT(c.overall_gba,0) as gba, FORMAT(c.overall_nra,0) as nra, FORMAT(c.inline_space_sf,0) as inlinesf, FORMAT(c.vacancy_sf_nra,0) as vacsfnra, CONCAT(FORMAT(c.oe_vacany_pct,1),'%') as vacpct, ot.definition as occupancy, c.no_stories as stories, c.const_descr as constdesc, bq.definition as bquality, bc.definition as building_class, ec.definition as extc, ic.definition as intc, c.other_const_features as constfeat, FORMAT(b.gross_land_sf,0) as grosssf, c.parking_ratio as pratio, c.land_build_ratio_primary as ltbprim, sa.definition as access, exp.definition as exposure, g.ind_clear_height as clearht, g.ind_truck_grade as grade, g.ind_truck_dock as dock, fext.definition as sprinkler, g.ind_hvac as hvac, rd.definition as rail, d.lessee_name as lessee, FORMAT(d.tenant_area_sf,0) as tenantsf, DATE_FORMAT(d.lease_start_date,'%c/%e/%Y') as startdate, IF(d.lease_start_comment = '','',CONCAT(' - ',d.lease_start_comment)) as startcomment, d.total_lease_term_mos as leaseterms, CONCAT(' ',mos.definition) as mos, d.lease_option_desc as optiondesc, let.definition as expterm, FORMAT(d.landlord_paid_exp_sf_yr,2) as llexpsfyr, d.landord_pays as llpays, d.tenant_paid_cam_sf_mo as camsfmo, FORMAT(c.office_bo_sf,0) as offsf, CONCAT(FORMAT(c.office_bo_pct,1),'%') as offbopct, nor.definition as renewal, sgen.definition as spacegen, spos.definition as spacepos, FORMAT(d.init_rent_sf_mo_blend,3) as initsfmoblend, CONCAT(FORMAT(d.init_rent_sf_mo_shell,3),' Shell') AS initsfmoshell, CONCAT(FORMAT(d.init_rent_sf_mo_office,3),' Office') AS initsfmooff, if(d.appraiser_estimates='3','', aest.definition) as appestimates, FORMAT(d.eff_rent_sf_mo_blend,3) as effsfmoblend, FORMAT(d.eff_rent_sf_mo_shell,3) as effsfmoshell, FORMAT(d.eff_rent_sf_mo_office,3) as effsfmooff, d.lease_esc_terms_desc as escdesc, d.concessions_desc as consessdesc, d.desc_ti as tidesc, IF(d.eff_rent_comment_1 = '','',CONCAT(' - ',d.eff_rent_comment_1)) AS effcomment, d.lease_comment as leasecomments, d.confirm_1_source as source1, d.relationship_1 as relationship1, d.confirm_2_souce as source2, IF(d.relationship_2 = '','',CONCAT(', ',d.relationship_2)) as relationship2, d.datasource, CONCAT(ap1.firstname, IF(ap1.midname = '','',CONCAT(' ',ap1.midname,' ')), ap1.lastname, IF(ap1.designation = '','',CONCAT(', ',ap1.designation))) as confirm1, CONCAT(ap2.firstname, IF(ap2.midname = '','',CONCAT(' ',ap2.midname,' ')), ap2.lastname, IF(ap2.designation = '','',CONCAT(', ',ap2.designation))) as confirm2, IF(ap1.digsignature = '','no-photo.jpg',CONCAT(ap1.id,'/',ap1.digsignature)) as apponesig, IF(ap2.digsignature = '','no-photo.jpg',CONCAT(ap2.id,'/',ap2.digsignature)) as apptwosig, DATE_FORMAT(d.confirm_date, '%c/%e/%Y') as confirmdate FROM report a JOIN property b on b.FK_ReportID = a.id LEFT OUTER JOIN building c on c.FK_ReportID = a.id LEFT OUTER JOIN leasetrans d on d.FK_ReportID = a.id LEFT OUTER JOIN proptypedetails g on g.FK_ReportID = a.id LEFT OUTER JOIN appraisers ap1 on ap1.id = d.confirm1 LEFT OUTER JOIN appraisers ap2 on ap2.id = d.confirm2 LEFT OUTER JOIN propertytype pt on pt.id = b.propertytype LEFT OUTER JOIN subtype st on st.id = b.propertysubtype LEFT OUTER JOIN WFDictionary rd on rd.id = g.ind_rail LEFT OUTER JOIN genmarket gm on gm.id = b.genmarket LEFT OUTER JOIN submarket sm on sm.id = b.submarkid LEFT OUTER JOIN WFDictionary ot on ot.id = c.occupancy_type LEFT OUTER JOIN WFDictionary bq on bq.id = c.building_quality LEFT OUTER JOIN WFDictionary bc on bc.id = c.building_class LEFT OUTER JOIN WFDictionary ec on ec.id = c.ext_condition LEFT OUTER JOIN WFDictionary fext on fext.id = g.off_fire_sprinkler LEFT OUTER JOIN WFDictionary ic on ic.id = c.int_condition LEFT OUTER JOIN WFDictionary sa on sa.id = b.site_access LEFT OUTER JOIN WFDictionary exp on exp.id = b.exposure LEFT OUTER JOIN WFDictionary ori on ori.id = b.orientation LEFT OUTER JOIN WFDictionary nor on nor.id = d.new_renewal LEFT OUTER JOIN WFDictionary sgen on sgen.id = d.space_generation LEFT OUTER JOIN WFDictionary spos on spos.id = d.space_position LEFT OUTER JOIN WFDictionary mos on mos.id = d.mos_vs_mos LEFT OUTER JOIN WFDictionary let on let.id = d.lease_expense_term LEFT OUTER JOIN WFDictionary aest on aest.id = d.appraiser_estimates WHERE a.id in (" . $_GET[ 'id' ] . ") group by FIELD(a.id," . $_GET[ 'id' ] . ") order by FIELD(a.id," . $_GET[ 'id' ] . ")" ) ) {
 
  $phpWord = new\ PhpOffice\ PhpWord\ PhpWord();
  $templateProcessor = new\ PhpOffice\ PhpWord\ TemplateProcessor( 'rentrowindustrial.docx' );
  ...
  while ( $record = $result->fetch_object() ) {    
    ...
    $templateProcessor->setValue( 'grade', $record->grade,1);
    $templateProcessor->setValue( 'llpays', $record->llpays,1);
    $templateProcessor->setValue( 'dock', $record->dock,1);
    $templateProcessor->setValue( 'offsf', $record->offsf,1);
    $templateProcessor->setValue( 'offbopct', $record->offbopct,1);
    $templateProcessor->setValue( 'initsfmoblend', $record->initsfmoblend,1);
    $templateProcessor->setValue( 'initsfmoshell', $record->initsfmoshell,1);
    $templateProcessor->setValue( 'initsfmooff', $record->initsfmooff,1);
    $templateProcessor->setValue( 'appestimates', $record->appestimates,1);
    $templateProcessor->setValue( 'effsfmoblend', $record->effsfmoblend,1);
    $templateProcessor->setValue( 'effsfmoshell', $record->effsfmoshell,1);
    $templateProcessor->setValue( 'effsfmooff', $record->effsfmooff,1);
    $templateProcessor->setValue( 'flexpctnra', $record->flexpctnra,1);
    $templateProcessor->setValue( 'clearht', $record->clearht,1);
    ...
    $templateProcessor->setValue( 'sprinkler', $record->sprinkler,1);
    ...
    $templateProcessor->setValue( 'camsfmo', $record->camsfmo,1); 
...
header( "Content-Disposition: attachment; filename=ROW Industrial Leases.docx" );
