...
WHERE a.id in (" . $_GET[ 'id' ] . ") group by FIELD(a.id," . $_GET[ 'id' ] . ") order by FIELD(a.id," . $_GET[ 'id' ] . ")" ) ) {
  ...
  $templateProcessor = new\ PhpOffice\ PhpWord\ TemplateProcessor( 'reshomerow.docx' );
  ...
  while ( $record = $result->fetch_object() ) {    
    ...
    $templateProcessor->setValue( 'nohomesite', $record->nohomesite,1);
    $templateProcessor->setValue( 'adjhomesite', $record->adjhomesite,1);
...
header( "Content-Disposition: attachment; filename=Residential Homesite Land Comps.docx" );
