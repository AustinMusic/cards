...
if ( $result = $conn->query( "SELECT @rn:=@rn+1 AS rank, saletype, CONCAT(price,' per Homesite', adjusted, saletype2, recdate) as price, salecomments
	FROM (
  	SELECT FORMAT(adjhomesite,0) as price, date_format(record_date,'%c/%y') as recdate, IF(sale_status = 185,'Listing ','Sale ') as saletype, IF(sale_status = 185,'Asking ',IF(sale_status = 181,'Exprired ',IF(sale_status = 182,'Sale-Fail ',IF(sale_status = 183,'Pending ',IF(sale_status = 186,'Purchase Option ',IF(sale_status = 188,'Terminated ',IF(sale_status = 445,'Assemblage ',''))))))) as saletype2, sale_comments as salecomments, IF(fin_term_adj + cond_sale_adj + demo_cost + onsite_extra_cost + offsite_develop + broker_cost + less_alloc_imp_price <>0,' - Adjusted; ','; ') as adjusted
  	FROM saletrans
