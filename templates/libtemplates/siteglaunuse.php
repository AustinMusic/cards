...
where a.id = " . $_GET['id'] . "" ) ) {
  ...
  $templateProcessor = new\ PhpOffice\ PhpWord\ TemplateProcessor( '../output/libtemps/siteglaunuse.docx' );
  ...
  while ( $record = $result->fetch_object() ) {
    ...
    $templateProcessor->setValue( 'unacre', $record->unacre );
    $templateProcessor->setValue( 'unsf', $record->unsf );
    $templateProcessor->setValue( 'useacre', $record->useacre );
    $templateProcessor->setValue( 'usesf', $record->usesf );
...
header( "Content-Disposition: attachment; filename=Site Description - Gross Land Unusable Net Usable.docx" );
