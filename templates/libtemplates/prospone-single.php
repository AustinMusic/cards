...
if ( $result = $conn->query( "select a.id, DATE_FORMAT(a.DueDate,'%M %e, %Y') as DueDate, a.reportname, b.property_name, CONCAT(b.streetnumber,' ',b.streetname) as address, CONCAT(b.city,', ',b.state,'  ',b.zip_code) as citystatezip, DATE_FORMAT(c.eff_date_value,'%M %e, %Y') as eff_date_value, DATE_FORMAT(c.prosstab_dov,'%M %e, %Y') as prosstab_dov, if(c.estate_appraised = 3,'', d.definition)  as estapp, if(c.prosstab_est_app = 3,'', j.definition)  as stabestapp, e.compname as clicomp, e.address as cliadd, e.suite as clisuite, e.city as clicity, e.state as clistate, e.zipcode as clizip, if(e.salutation = 3,'',i.definition) as clisal, e.firstname as clifname, e.lastname as clilname, if(e.designation = '', e.designation, concat(', ',e.designation)) as clides, e.clienttitle as clititle, f.id as apponeid, f.firstname as apponefname, f.midname as apponemname, f.lastname as apponelname, IF(f.designation = '','', CONCAT(', ', f.designation)) as apponedes, if(f.apptitle = 3,'',h.definition) as apponetitle, f.emailaddress as apponeemail, f." . $_GET['licst'] . " as apponelicst, f." . $_GET['licno'] . " as apponelicno, IF(f.digsignature = '','../../app_images/no-photo.jpg',CONCAT('../../app_images/', f.id,'/',f.digsignature)) as apponedigsig
from report a
join clients e
join appraisers f
...
where a.id = " . $_GET['id'] . " and e.id = " . $_GET['client'] . " and f.id = " . $_GET['app'] . "" ) ) {
  ...
  $templateProcessor = new\ PhpOffice\ PhpWord\ TemplateProcessor( '../output/libtemps/prospone-single.docx' );
