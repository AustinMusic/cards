<?php
$templatePath = dirname(__FILE__).'/../output/libtemps/scoperetro.docx';
$outputFilename = 'Scope - Retrospective.docx';
...
if ($result = $conn->query("select a.id, a.reportname, UPPER(b.property_name) as cappropname, DATE_FORMAT(c.inspect_date,'%M %e, %Y') as insdate, DATE_FORMAT(c.retro_dov,'%M %e, %Y') as retrodov from report a left join property b on b.FK_ReportID = a.id left join appraisalinfo c on c.FK_ReportID = a.id where a.id = " . $_GET['id'] . "")) {
    ...
    if($record != NULL) {
        ...
        $templateProcessor->setValue( 'retrodov', $record->retrodov );
    } else {
