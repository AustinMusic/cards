...
if ( $result = $conn->query( "select a.id, a.reportname, UPPER(b.property_name) as cappropname, b.property_name, format(c.overall_nra,0) as nra, format(c.overall_gba,0) as gba, format(d.ind_int_office_1,0) as off1, format(d.ind_int_office_2,0) as off2, format(sum(d.ind_int_office_1 + d.ind_int_office_2),0) as offsf, concat(d.ind_pct_office,'%') as offpct, d.ind_clear_height as clearh, c.parking_stalls as pspaces, c.parking_ratio as pratio, concat(c.year_built, CONCAT(' ',c.last_renovation)) as yearbuilt, concat(c.site_cover_overall,'%') as sitegba, concat(c.site_cover_primary,'%') as siteprim, c.land_build_ratio_primary as landprim, c.land_build_ratio_overall as landover, format(c.floor_1_gba,0) as footprint, format(b.gross_land_sf,0) as glasf, format(b.primary_sf,0) as primsf
from report a
...
where a.id = " . $_GET['id'] . "" ) ) {
  ...
  $templateProcessor = new\ PhpOffice\ PhpWord\ TemplateProcessor( '../output/libtemps/idstindsy.docx' );
  ...
  while ( $record = $result->fetch_object() ) {
    ...
    $templateProcessor->setValue( 'off1', $record->off1 );
    $templateProcessor->setValue( 'off2', $record->off2 );
...
header( "Content-Disposition: attachment; filename=Improv Descrip - Single-Tenant Indust with Surplus Yard.docx" );
