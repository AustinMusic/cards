...
where a.id = " . $_GET['id'] . "" ) ) {
  ...
  $templateProcessor = new\ PhpOffice\ PhpWord\ TemplateProcessor( '../output/libtemps/recasisfeesimp.docx' );
...
header( "Content-Disposition: attachment; filename=Reconcliation - As Is - Fee Simple.docx" );
readfile( $temp_file );
unlink( $temp_file );
