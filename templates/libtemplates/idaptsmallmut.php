...
if ( $result = $conn->query( "select a.id, a.reportname, UPPER(b.property_name) as cappropname, b.property_name, format(c.no_units,0) as units, format(c.parking_ratio_unit,1) as pratunit, format(c.overall_nra,0) as nra, format(c.overall_gba,0) as gba, format(c.floor_1_gba,0) as footprint, c.parking_stalls as pspaces, c.parking_ratio as pratio, c.year_built as yearb, concat(c.year_built, CONCAT(' ',c.last_renovation)) as yearbuilt, concat(c.site_cover_overall,'%') as sitegba, concat(c.site_cover_primary,'%') as siteprim, c.land_build_ratio_primary as landprim, format(b.primary_sf,0) as primsf
from report a
...
where a.id = " . $_GET['id'] . "" ) ) {
  ...
  $templateProcessor = new\ PhpOffice\ PhpWord\ TemplateProcessor( '../output/libtemps/idaptsmallmut.docx' );
  ...
  while ( $record = $result->fetch_object() ) {
    ...
    $templateProcessor->setValue( 'units', $record->units );
    ...
    $templateProcessor->setValue( 'pratunit', $record->pratunit );
...
header( "Content-Disposition: attachment; filename=Improv Descrip - Apartment - Small Multi Unit Type.docx" );
