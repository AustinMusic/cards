<?php
if ($_GET['apptwo'] !=''){
    $templatePath = dirname(__FILE__).'/../output/libtemps/prosptwo-dual.docx';
    } else {
    $templatePath = dirname(__FILE__).'/../output/libtemps/prosptwo-single.docx';
}
$outputFilename = 'Prospective - At Completion Transmittal Letter.docx';
...
if ($result = $conn->query("SELECT a.id, UPPER(b.property_name) as cappropname, b.property_name as propname, b.owner_name as owner, a.reportname, CONCAT(b.streetnumber,' ',b.streetname) as address, CONCAT(b.city,', ',b.state,'  ',b.zip_code) as citystatezip, b.county, DATE_FORMAT(a.DueDate,'%M %e, %Y') as DueDate, DATE_FORMAT(c.inspect_date,'%M %e, %Y') as insDate, c.client_reference_no as clientref, c.borrower, DATE_FORMAT(c.eff_date_value,'%M %e, %Y') as effdov, if(c.estate_appraised = 3,'', d.definition) as estapp, DATE_FORMAT(c.proscomp_dov,'%M %e, %Y') as compdov, if(c.proscomp_est_app = 3,'', e.definition) as compapp, DATE_FORMAT(c.prosstab_dov,'%M %e, %Y') as stabdov, if(c.prosstab_est_app = 3,'', f.definition) as stabapp FROM report a LEFT JOIN property b on b.FK_ReportID = a.id JOIN appraisalinfo c on c.FK_ReportID = a.id JOIN WFDictionary d on d.id = c.estate_appraised JOIN WFDictionary e on e.id = c.proscomp_est_app JOIN WFDictionary f on f.id = c.prosstab_est_app WHERE a.id = " . $_GET['id'] . "")) {
    ...
    if($record != NULL) {
        ...
        $templateProcessor->setValue( 'stabdov', $record->stabdov);
        $templateProcessor->setValue( 'stabapp', $record->stabapp);
        $templateProcessor->setValue( 'compdov', $record->compdov);
        $templateProcessor->setValue( 'compapp', $record->compapp);
