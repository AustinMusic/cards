<?php
if ($_GET['apptwo'] !=''){
    $templatePath = dirname(__FILE__).'/../output/libtemps/asis-dual.docx';
    } else {
    $templatePath = dirname(__FILE__).'/../output/libtemps/asis-single.docx';
}
$outputFilename = 'As Is Transmittal Letter.docx';
...
if ($result = $conn->query("SELECT a.id, UPPER(b.property_name) as cappropname, b.property_name as propname, b.owner_name as owner, a.reportname, CONCAT(b.streetnumber,' ',b.streetname) as address, CONCAT(b.city,', ',b.state,'  ',b.zip_code) as citystatezip, b.county, DATE_FORMAT(a.DueDate,'%M %e, %Y') as DueDate, DATE_FORMAT(c.inspect_date,'%M %e, %Y') as insDate, c.client_reference_no as clientref, c.borrower, DATE_FORMAT(c.eff_date_value,'%M %e, %Y') as effdov, if(c.estate_appraised = 3,'', d.definition) as estapp FROM report a LEFT JOIN property b on b.FK_ReportID = a.id JOIN appraisalinfo c on c.FK_ReportID = a.id JOIN WFDictionary d on d.id = c.estate_appraised WHERE a.id = " . $_GET['id'] . "")) {
