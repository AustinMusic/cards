...
where a.id = " . $_GET['id'] . "" ) ) {
  ...
  $templateProcessor = new\ PhpOffice\ PhpWord\ TemplateProcessor( '../output/libtemps/recstabasisfeesimp.docx' );
...
header( "Content-Disposition: attachment; filename=Reconcliation - Stabilized As Is - Fee Simple.docx" );
readfile( $temp_file );
unlink( $temp_file );
