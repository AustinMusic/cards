<?php
$templatePath = dirname(__FILE__).'/../output/libtemps/introasispros.docx';
$outputFilename = 'Introduction - As Is Prospective.docx';
...
if ($result = $conn->query("select a.id, a.reportname, DATE_FORMAT(a.DueDate,'%M %e, %Y') as DueDate, UPPER(b.property_name) as cappropname, b.property_name, CONCAT(b.streetnumber,' ',b.streetname) as address, CONCAT(b.city,', ',b.state,'  ',b.zip_code) as citystatezip, b.legal_desc, c.client_name, DATE_FORMAT(c.eff_date_value,'%M %e, %Y') as eff_date_value, DATE_FORMAT(c.prosstab_dov,'%M %e, %Y') as prosstab_dov, DATE_FORMAT(c.proscomp_dov,'%M %e, %Y') as proscomp_dov, DATE_FORMAT(c.inspect_date,'%M %e, %Y') as inspect_date, IF(c.estate_appraised = 3,'',d.definition) as estateapp, if(c.prosstab_est_app = 3,'',e.definition) as prosstabestapp, IF(c.proscomp_est_app = 3,'',f.definition) as proscompestapp from report a join property b on b.FK_ReportID = a.id join appraisalinfo c on c.FK_ReportID = a.id join WFDictionary d on d.id = c.estate_appraised join WFDictionary e on e.id = c.prosstab_est_app join WFDictionary f on f.id = c.proscomp_est_app where a.id = " . $_GET['id'] . "")) {
