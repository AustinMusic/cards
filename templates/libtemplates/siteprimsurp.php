...
if ( $result = $conn->query( "select a.id, a.reportname, UPPER(b.property_name) as cappropname, concat(b.streetnumber,' ',b.streetname) as address, concat(b.city,', ',b.state,' ',b.zip_code) as cityst, b.city, b.county, if(b.state = 'ID','Idaho',if(b.state = 'WA','Washington','Oregon')) as state, format(b.gross_land_acre,2) as glaacre, format(b.gross_land_sf,0) as glasf, format(b.primary_acre,2) as primacre, format(b.primary_sf,0) as primsf, format(b.surplus_acre,2) as suracre, format(surplus_sf,0) as sursf, c.definition as shape, format(d.floor_1_gba,0) as foot, concat(format(d.site_cover_primary,1),'%') as site
from report a
...
where a.id = " . $_GET['id'] . "" ) ) {
  ...
  $templateProcessor = new\ PhpOffice\ PhpWord\ TemplateProcessor( '../output/libtemps/siteprimsurp.docx' );
...
header( "Content-Disposition: attachment; filename=Site Description - Primary Site - Surplus Land.docx" );
